...
router.get("/", function(req, res) {
  ...
  if (req.session.user_id) {
    res.redirect("/waterlineLogin/getUser");
  }
  res.sendFile(path.join(__dirname+"/../public/waterlineUser/login.html"));  //로그인페이지로 보냄
});


router.get("/createUser", function(req, res) {
  console.log("회원가입 페이지 출력");
  res.sendFile(path.join(__dirname+"/../public/waterlineUser/createUser.html"));  //회원가입페이지로 보냄
});
...
router.get("/getUser", function(req, res) {
  ...
  if (!req.session.user_id) {  //세션이 존재하지 않으면
    res.redirect("/waterlineLogin");  //로그인페이지로 보냄
  }
  res.sendFile(path.join(__dirname+"/../public/waterlineUser/read&updateUser.html"));
});
